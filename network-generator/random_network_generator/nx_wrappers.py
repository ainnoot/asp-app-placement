...
class ErdosRenyi(GraphGenerator):
    def __init__(self, **kwargs):
        ...
        assert "p" in kwargs
        assert "seed" not in kwargs
        super().__init__(nx.generators.gnp_random_graph, **kwargs)
...
class CompleteGraph(GraphGenerator):
    def __init__(self, **kwargs):
        assert "n" in kwargs
        self.n = kwargs["n"]
        super().__init__(nx.generators.complete_graph, **kwargs)

    def generate(self, **kwargs):
        return nx.generators.complete_graph(self.n, nx.DiGraph())
